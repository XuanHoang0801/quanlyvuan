<?php
    ...
    if(empty( $_POST ['name']  ) == true)
        ...
        else
        {
            ...
            //thêm
            $insert = "INSERT INTO tbl_donvi(DonVi_VuAn) VALUES ('".$donvi."')";
                $add = mysqli_query($conn,$insert);
                if($add)
                     {
                        echo '<script language="javascript">alert("Thêm đơn vị thành công !"); window.location="unit.php";</script>';
                     }
                else{
                        echo '<script language="javascript">alert("Thêm đơn vị thất bại !"); window.location="add-unit.php";</script>';
                    }
        }
    }
